...
# basemap is centered on the extent of the original tiff files.
...
for (i in seq_along(transparent_pngs)) {
  ...
  # Grab new ice image dimensions
  scaled_w <- image_info(scaled_image)$width
  scaled_h <- image_info(scaled_image)$height
